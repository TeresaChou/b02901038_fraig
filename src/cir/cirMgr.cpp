...
// remove unused gates
bool
CirMgr::removeGate(unsigned id, bool free)
{
	CirGate* target = getGate(id);
	if(!target->isAig())	return false;
	// remove the target from the fanout list of its fanins
	target->clearFanin();
	// remove the target from the AigList and gateList
	for(vector<unsigned>::iterator it = _AigList.begin(); it!=_AigList.end(); it++)
		if(*it == id)	{ _AigList.erase(it); break; }
	if(free)		delete target;		// free the gate
	cout << "AIG(" << id << ") removed..." << endl;
	if(free)	_gateList.erase(id);
	return true;
}
